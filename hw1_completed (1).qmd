...
clt_list <- lapply(sample_sizes, function(s) {
  ...
  data.frame(n = s, theta_hat = thetas,
             n_label = paste0("n = ", s))
})
...
ggplot(clt_data, aes(x = theta_hat)) +
  ...
  geom_vline(xintercept = 0.04, color = "#EF4444",
...
hyp_df <- data.frame(
  ...
  Value     = c(round(theta_hat, 4), round(analytic_se, 4),
                round(z_stat, 3), formatC(p_value, format = "g", digits = 3))
)
...
false_positives <- replicate(n_sims, {
  ...
  any(sapply(peek_ns, function(k) {
    ...
    se <- sqrt(mean(A[1:k]) * (1 - mean(A[1:k])) / k +
...
knitr::kable(peek_df, caption = "The Cost of Peeking (H₀ true: πA = πB = 0.20)",
             align = "lr")
```
